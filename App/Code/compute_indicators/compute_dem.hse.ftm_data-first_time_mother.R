# Compute Indicator
# Extract data for First Time Mother from Birth Risk indicators
...
compute_ftm_fig1 <- function(data) {
  data %>%
    filter(indicator == "First Time Mother",
           is.na(group)) %>%
    mutate(dimension = "Demographic",
           measure = "Household Characteristics",
           subset_level = "none",
           group_level = "none") 
}

# Function to extract data for Fig 3
compute_ftm_fig3 <- function(data) {
  data %>%
    filter(indicator == "First Time Mother",
           !is.na(group)) %>%
    mutate(dimension = "Demographic",
...
for (i in unique(my.data.1$geo)) {
  ...
  file_name <-
    switch (i,
            "data_ia"        = paste0("hse_ftm_fig1_2_", "co", ".csv"),
            "data_eci"       = paste0("hse_ftm_fig1_2_", "eci", ".csv"),
            "data_hhs"       = paste0("hse_ftm_fig1_2_", "rg", ".csv"),
            "data_hs"        = paste0("hse_ftm_fig1_2_", "hs", ".csv"),
            "data_miechv_26" = paste0("hse_ftm_fig1_2_", "mv", ".csv")
    )
  ...
  my.data.1 %>%
    ...
    filter(geo == i) %>%
    select(-geo) %T>%
    write_csv(file.path(cy_path, cy_idd_path, "demographic indicators", file_name)) %>%
    write_csv(file.path(cy_path, cy_my_path, "Demographic", dir_name, file_name))
}


# Save data separately for each geography for Fig 3 and 4
for (i in unique(my.data.3$geo)) {
  # set file name for saving data
  file_name <-
    switch (i,
            "data_ia"        = paste0("hse_ftm_fig3_4_", "co", ".csv"),
            "data_eci"       = paste0("hse_ftm_fig3_4_", "eci", ".csv"),
            "data_hhs"       = paste0("hse_ftm_fig3_4_", "rg", ".csv"),
            "data_hs"        = paste0("hse_ftm_fig3_4_", "hs", ".csv"),
            "data_miechv_26" = paste0("hse_ftm_fig3_4_", "mv", ".csv")
    )
  # set directory name to save data in my CyBox
  dir_name <- 
    switch (i,
